...
def csv2npy(df_all, pwd, split, stratify=None, random_states=0):
    ...
    np.save(os.path.join(pwd, "train","%s_train.npy"%split), train_df.to_numpy())
    np.save(os.path.join(pwd, "valid","%s_valid.npy"%split), valid_df.to_numpy())
    np.save(os.path.join(pwd, "test","%s_test.npy"%split),   test_df.to_numpy())
    # 保存索引为独立的文件
    np.save(os.path.join(pwd, "train", "%s_train_index.npy" % split), train_df.index.to_numpy())
    np.save(os.path.join(pwd, "valid", "%s_valid_index.npy" % split), valid_df.index.to_numpy())
    np.save(os.path.join(pwd, "test", "%s_test_index.npy" % split), test_df.index.to_numpy())
...
filename1 = "data/elements.csv"
filename2 = "data/positions.csv"
filename3  = "data/tgtdos.csv"
